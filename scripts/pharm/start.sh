...
for target in [a-z]*/; do
    if [ "$counter" -lt "$maxrun" ]; then
        ...
        if [ ! -f "${target}/run${1}/haddock.out" ]; then
            ...
            if [ ! -f "${target}/run${1}/structures/it1/file.nam" ]; then
                ...
                check=`tail -100 "${target}/run${1}/haddock.out" | grep -c 'failed structures in it0'`
                if [ "$check" -gt "0" ]; then
                ...
                check=`tail -100 "${target}/run${1}/haddock.out" | grep -c 'failed structures in it1' | grep -c '>20%'`
                if [ "$check" -gt "0" ]; then
                ...
                # Check for failed structures in it1
                check=`tail -100 "${target}/run${1}/haddock.out" | grep -c 'failed structures in it1'`
                if [ "$check" -gt "0" ]; then
                ...
                now=`date +%s`
                last_mod_time=`date -r ${target}/run${1}/haddock.out +%s`
